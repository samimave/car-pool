...
*/	
	...
	public void testRideListing(){
		...
		try {
			while(rl.next()){
				rideCount++;
				System.out.println(rl.getRideID()+", "+rl.getUserID()+", "+rl.getUsername()+", "+rl.getAvailableSeats()+", "+rl.getRideDate().toString()+", "+rl.getStartLocation()+", "+rl.getEndLocation() + ", " + rl.getComment());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void testSearchRideListing(){
		...
		try {
			while(rl.next()){
				System.out.println(rl.getRideID()+", "+rl.getUserID()+", "+rl.getUsername()+", "+rl.getAvailableSeats()+", "+rl.getRideDate().toString()+", "+rl.getTime()+", "+rl.getStartLocation()+", "+rl.getEndLocation()+" geoLocation:" + rl.getGeoLocation() + " comment:" + rl.getComment());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
...
//			int ride5 = cps.addRide(e, 4, date.toString(), "asgadfg", "adfgadfgafd home");
	...
	public void testHasUserAddedScore(){
...
//			int ride5 = cps.addRide(e, 4, date.toString(), "asgadfg", "adfgadfgafd home");
			...
			assertEquals(false, cps.hasUserAddedScore(ride1, e));
			cps.addScore(cps.getTripID(ride1, e), a, 3);
			assertEquals(true, cps.hasUserAddedScore(ride1, e));
			
			assertEquals(false, cps.hasUserAddedScore(ride1, f));
			cps.addScore(cps.getTripID(ride1, f), a, 3);
			assertEquals(true, cps.hasUserAddedScore(ride1, f));
			
			assertEquals(false, cps.hasUserAddedScore(ride1, g));
			cps.addScore(cps.getTripID(ride1, g), a, 3);
			assertEquals(true, cps.hasUserAddedScore(ride1, g));
			
			assertEquals(false, cps.hasUserAddedScore(ride1, h));
			cps.addScore(cps.getTripID(ride1, h), a, 3);
			assertEquals(true, cps.hasUserAddedScore(ride1, h));
...
*/			
			Date date = new Date(System.currentTimeMillis());
			//addRide(int user, int availableSeats, String startDate, int startLocation, int endLocation, int streetNumber, int reoccur, String time, String comment)			
			int ride1 = cps.addRide(a, 4, date.toString(), idLocation, idLocation, 0,1 , 0, "5:22 PM", "be on time", "start0/end0");
			int ride2 = cps.addRide(b, 4, date.toString(), idLocation, idLocation,1,1 , 0, "6:22 AM", "no latecomers please", "start0/end0");
			int ride3 = cps.addRide(c, 4, date.toString(), idLocation, idLocation,2,1 , 0, "3:22 PM", "I might be a few minutes late", "start0/end0");
			int ride4 = cps.addRide(d, 4, date.toString(), idLocation, idLocation,3,1 , 0, "9:22 PM", "be on time", "start0/end0");
//			int ride5 = cps.addRide(e, 4, date.toString(), "asgadfg", "adfgadfgafd home");
