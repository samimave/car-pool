...
<%@page import="car.pool.persistance.test.AddLocations"%><HTML>
	<HEAD>
		...
		// EXCEPTIONS
		//
		// Handle exceptions for sites in Massey DNS domain but outside Massey
		network
		//
		  if ( host == "digitalcommons.massey.ac.nz" || host ==
		"digitalcommons") {
		     ret = URLiphash(myIpAddress());
		     if ( (ret % 2) == 1 ) {
		        return "PROXY tur-cache1.massey.ac.nz:8080; PROXY tur-
		cache2.massey.ac.nz:8080";
		     } else  {
		        return "PROXY tur-cache2.massey.ac.nz:8080; PROXY tur-
		cache1.massey.ac.nz:8080";
		     }
		  }
		//
		// EVERYONE ELSE
		//
		// If you are going to a Massey site or you haven't appended a domain,
		then
		// you can go direct
		//
		  if (isPlainHostName(host) || dnsDomainIs(host, ".massey.ac.nz") ||
		dnsDomainIs(host, ".mass-e-mall.co.nz") || dnsDomainIs(host, ".mass-e-
		mall.com")) {
		     return "DIRECT;";
		  }
		...
		//
		  if (isInNet(myIpAddress(), "130.123.224.0", "255.255.224.0") ||
		isInNet(myIpAddress(), "10.101.224.0", "255.255.254.0")) {
		     return "PROXY alb-cache.massey.ac.nz:8080; PROXY tur-
		cache1.massey.ac.nz:8080";
		  }
		...
		//
		  if (isInNet(myIpAddress(), "130.123.192.0", "255.255.224.0")  ||
		isInNet(myIpAddress(), "10.101.192.0", "255.255.254.0")) {
		     return "PROXY wel-cache.massey.ac.nz:8080; PROXY tur-
		cache2.massey.ac.nz:8080";
		  }
		...
		//
		  if (isInNet(myIpAddress(), "130.123.4.0", "255.255.252.0")) {
		     return "PROXY tur-cache1.massey.ac.nz:8080; PROXY alb-
		cache.massey.ac.nz:8080";
		  }
		//
		// Singapore
		//
		...
		//
		// If you aren't in Albany, Hokowhitu or Wellington, use the caches at
		Turitea
		//
		  ret = URLiphash(myIpAddress());
		  if ( (ret % 2) == 1 ) {
